dataDownload=function(variables=NULL,type="cia",SKUS=NULL, depart=c("D040")){
  ...
  if(type=="opened-stores"){
   ...
   if(length(remove)!=0){
    ...
    aux1 = paste(aux1, SKUS[length(SKUS)])
    p1 = c(" SELECT GEO_CD_LOJA, to_char(PER_DT_DATA, 'yyyy-mm-dd') as PER_DT_DATA,")
    p2 = c("FROM RPO_VDAEST_TRANSACIONAL WHERE PROD_CD_ITEM IN (")
    p3 = c(") GROUP BY GEO_CD_LOJA, PER_DT_DATA")
    sql = paste(p1, aux, p2, aux1, p3)
    query = dbSendQuery(con, paste(sql))
    query = fetch(query, n = -1)
    taux = Sys.time()
    ...
    n.cores = min(length(variables), 32)
    cl = makeCluster(n.cores)
    tables = parLapply(cl, aux, te, range = range, table = table, 
                       indexes = indexes)
    names(tables) = variables
    stopCluster(cl)
    t1 = Sys.time()
    sqltime = (t1 - t0)
    cat("Data successfuly extracted. Processing time of", 
        sqltime, "\n")
    return(tables)
  }
